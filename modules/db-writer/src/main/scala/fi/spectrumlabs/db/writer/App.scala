...
import fi.spectrumlabs.db.writer.schema.AnotherExampleSchema
  ...
  def init(configPathOpt: Option[String]): Resource[InitF, Unit] =
    for {
      ...
      implicit0(elh: EmbeddableLogHandler[xa.DB]) <-
        Resource.eval(doobieLogging.makeEmbeddableHandler[InitF, RunF, xa.DB]("db-writer-db-logging"))
      implicit0(logsDb: Logs[InitF, xa.DB]) = Logs.sync[InitF, xa.DB]

      implicit0(consumer: Consumer[String, Transaction, StreamF, RunF]) <-
        makeConsumer[String, Transaction](configs.tnxConsumer, configs.kafka)

      schemaExample = Persist.create[Transaction, ExampleData, xa.DB]
      schemaAnotherExample = Persist.create[Transaction, AnotherExampleSchema, xa.DB]
