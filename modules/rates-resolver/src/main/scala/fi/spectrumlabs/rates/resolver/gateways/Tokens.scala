...
import fi.spectrumlabs.rates.resolver.models.{TokenInfo, TokenList}
...
object Tokens {
  ...
  def create[I[_]: Sync, F[_]: Sync: Throws: Clock](config: NetworkConfig)(implicit
    backend: SttpBackend[F, _],
    logs: Logs[I, F]
  ): I[Tokens[F]] =
    for {
      cache                       <- Ref.in[I, F, (Long, List[TokenInfo])](0L -> List.empty)
      implicit0(logs: Logging[F]) <- logs.forService[Network[F]]
    } yield new Tracing[F] attach new Impl[F](config, cache)

  final private class Impl[F[_]: Monad: Throws: Clock](config: NetworkConfig, cache: Ref[F, (Long, List[TokenInfo])])(
